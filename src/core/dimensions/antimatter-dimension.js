...
function onBuyDimension(tier) {
  ...
  if (tier === 6) LogicNode.galaxyNerf.tryUnlock();
...
export function buyOneDimension(tier) {
  ...
  if (Laitela.continuumActive || !dimension.isAvailableForPurchase || !dimension.isAffordable || !dimension.hasDLC || !dimension.isUnlocked) return false;
...
export function buyManyDimension(tier) {
  const dimension = AntimatterDimension(tier);
  if (Laitela.continuumActive || !dimension.isAvailableForPurchase || !dimension.isAffordableUntil10 || !dimension.hasDLC || !dimension.isUnlocked) return false;
...
function buyUntilTen(tier) {
  const dimension = AntimatterDimension(tier);
  if (Laitela.continuumActive || !dimension.hasDLC || !dimension.isUnlocked) return;
  dimension.challengeCostBump();
...
export function buyMaxDimension(tier, bulk = Infinity) {
  const dimension = AntimatterDimension(tier);
  if (Laitela.continuumActive || !dimension.isAvailableForPurchase || !dimension.isAffordableUntil10 || !dimension.hasDLC || !dimension.isUnlocked) return;
  const cost = dimension.costUntil10;
  let bulkLeft = bulk;
...
class AntimatterDimensionState extends DimensionState {
  ...
  get hasDLC() {
    return player.hasDLC || this.tier === 1;
  }
  ...
  get productionPerSecond() {
    ...
    if (tier === 1) {
      ...
      if (production.gt(10)) {
        ...
        production = Decimal.pow10(Math.pow(log10,
...
export const AntimatterDimensions = {
  ...
  get buyTenMultiplier() {
    ...
    let mult = new Decimal(Effects.max(
      2,
  ...
  tick(diff) {
    ...
    if (InfinityChallenge(11).isRunning) {
      for (let tier = Puzzles.maxTier; tier >= 1; --tier) {
