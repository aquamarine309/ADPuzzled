...
- <b>AG</b>: Antimatter Galaxy
${Achievement(26).config.hintCondition() ? "<button class='o-primary-btn o-buy-galaxy-button' onclick='forceGalaxyReset()'>Purchase</button>" : ""}<br>
...
<br>
<br>
You start with 1 Logic Point. To increase Logic Points, you may exchange at most ${formatInt(6)} types of resources for different multipliers to the total Logic Points. You can level up Exchange to unlock a new exchangeable resource. As long as you get enough Logic Points, just level up Exchange at once because it gives you a huge boost.
<br>
<br>
The Logic Upgrades below, similar to the Reality Upgrades, requires you to reach certain condition to unlock. In addition, once unlocked, it will never be locked again when you Infinity. Then, you can use Logic Points to buy the unlocked upgrades. Currently, there are ${formatInt(10)} Logic Upgrades, which are all shown as soon as you Big Crunch for the first time. The last Logic Upgrade unlocks Logic Challenges.
`,
      isUnlocked: () => PlayerProgress.infinityUnlocked(),
